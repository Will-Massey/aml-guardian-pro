...
const apiKey = Buffer.from(config.companiesHouse.apiKey).toString('base64');
...
class CompaniesHouseService {
  ...
  constructor() {
    ...
    this.client.interceptors.response.use(
      ...
      (error: AxiosError) => {
        if (error.response) {
          logger.warn('Companies House API error', {
   ...
   */
  async searchCompanies(query: string, itemsPerPage: number = 20): Promise<CHSearchItem[]> {
    try {
      if (!query || query.length < 2) {
        return [];
      }

      const response = await this.client.get<CHSearchResult>('/search/companies', {
        params: {
          q: query,
      ...
      logger.info('Companies House search completed', {
        query,
   ...
   */
  async getCompanyDetails(companyNumber: string): Promise<CHCompany | null> {
    try {
      const cleanNumber = this.cleanCompanyNumber(companyNumber);
      
      const response = await this.client.get<CHCompany>(`/company/${cleanNumber}`);
      
      logger.info('Retrieved company details', {
   ...
   */
  async getCompanyOfficers(companyNumber: string): Promise<Officer[]> {
    try {
      const cleanNumber = this.cleanCompanyNumber(companyNumber);
      
      const response = await this.client.get<{ items: any[] }>(
        `/company/${cleanNumber}/officers`
      );

      const officers = (response.data.items || []).map(this.mapOfficer);
      
      logger.info('Retrieved company officers', {
        companyNumber: cleanNumber,
      ...
      logger.error('Failed to get company officers', error);
      return [];
    }
  }
   ...
   */
  async getCompanyPSCs(companyNumber: string): Promise<PSC[]> {
    try {
      const cleanNumber = this.cleanCompanyNumber(companyNumber);
      
      const response = await this.client.get<{ items: any[] }>(
        `/company/${cleanNumber}/persons-with-significant-control`
      );

      const pscs = (response.data.items || []).map(this.mapPSC);
      
      logger.info('Retrieved company PSCs', {
        companyNumber: cleanNumber,
        count: pscs.length,
      });
      ...
      // Some companies may not have PSCs endpoint available
      if (axios.isAxiosError(error) && error.response?.status === 410) {
        return [];
      }
      logger.error('Failed to get company PSCs', error);
      return [];
    }
  }

  /**
   * Get full company profile including officers and PSCs
   */
  async getFullCompanyProfile(companyNumber: string): Promise<{
    ...
    const [company, officers, pscs] = await Promise.all([
      this.getCompanyDetails(cleanNumber),
      this.getCompanyOfficers(cleanNumber),
      this.getCompanyPSCs(cleanNumber),
    ]);
   ...
   */
  private cleanCompanyNumber(number: string): string {
    // Remove spaces and convert to uppercase
    return number.replace(/\s/g, '').toUpperCase();
  }

  /**
   * Map CH address to our Address type
   */
  mapAddress(chAddress: any): Address {
    return {
      addressLine1: chAddress.address_line_1 || '',
      addressLine2: chAddress.address_line_2,
  ...
  /**
   * Map CH officer to our Officer type
   */
  private mapOfficer(officer: any): Officer {
    return {
      name: officer.name,
      role: officer.officer_role || officer.role,
      ...
      address: officer.address ? this.mapAddress(officer.address) : undefined,
    };
  }

  /**
   * Map CH PSC to our PSC type
   */
  private mapPSC(psc: any): PSC {
    let name: string;
    
    // Handle different PSC types (individual vs corporate)
    if (psc.name) {
      name = psc.name;
   ...
   */
  getSICCodeDescription(code: string): string {
    const descriptions: Record<string, string> = {
      ...
      '01629': 'Support activities for animal production (other than farm animal boarding and care)',
      ...
      '10833': 'Manufacture of condiments and seasonings',
      '10840': 'Manufacture of prepared meals and dishes',
      '10850': 'Manufacture of homogenized food preparations and dietetic food',
      '10860': 'Manufacture of other food products n.e.c.',
      ...
      '28302': 'Manufacture of agricultural and forestry machinery',
      ...
      '31010': 'Manufacture of office furniture',
      ...
      '46439': 'Wholesale of radio, television and other electrical household appliances (other than records, tapes, CDs and video tapes and the equipment on which these are played)',
      ...
      '78109': 'Other activities of employment placement agencies (not including motion picture, television casting)',
      '78200': 'Temporary employment agency activities',
      '78300': 'Human resources provision and management of human resources functions',
      '79110': 'Activities of tourist guides',
      '79120': 'Tour operator activities',
      '79901': 'Activities of tour agents',
    ...
    return descriptions[code] || `SIC Code ${code}`;
  }
}
